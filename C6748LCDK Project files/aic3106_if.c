 ...
 *  DSP BIOS operating system. This file includes the ADC and DAC configuration related API's of the CODEC.
 *  You can configure the registers setting of the CODEC here by using the < CodecRegWrite()/ Read > functions.
 *  The sample rate of the ADC and DAC are configured here but to change the sample rate please do not change
 *  anything here, you can directly pass a struct containing the sample rate value while creating the channel.
 *
 *
 ...
 * All the ADC related register setting is done here.
 *
 * \param   baseAddr     Base Address of the interface connected to AIC31
 *
 * \return  None.
...
void AIC31ADCInit(unsigned int baseAddr)
{
    ...
    /* My code and configuration here*/
    CodecRegWrite(baseAddr, AIC31_P0_REG12, 0xFA);
}
